...
# regenere le matricule
def generer_matricule(departement, vacation):
    annee = datetime.now().year
    code_departement = departement.initiale.upper()
    code_vacation = vacation.upper()

    dernier = Etudiants.objects.order_by('-id').first()
    
    if dernier and dernier.matricule:
        try:
            dernier_num = int(dernier.matricule[-3:])
    ...
    matricule = f"{annee}{code_departement}{code_vacation}{nouveau_num:03d}"
    return matricule
# 
...
def createEtudiant(request):
    ...
    if request.method == 'POST':
        try:
            ...
            matricule = generer_matricule(departement_id, vacation)
